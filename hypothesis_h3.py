...
def create_healthcare_coverage_chart(df):
    """
    Create subplots showing healthcare coverage indicators with dropdown to filter by income level.
    Displays side-by-side comparison of healthcare coverage and cost barriers for each income level.
    """
    df = df.copy()
    ...
    # Income order
    income_order = ['< $10k', '$10k-$15k', '$15k-$20k', '$20k-$25k', '$25k-$35k', '$35k-$50k', '$50k-$75k', '> $75k']
    ...
    # Create all traces and buttons
    all_traces = []
    buttons_list = []
    
    for income_idx, income_level in enumerate(income_order):
        ...
        all_traces.append(go.Bar(
            ...
            showlegend=False,
            visible=(income_idx == 4),  # Default to index 4 ($25k-$35k)
        ...
        # Create button for this income level
        visibility = [False] * (len(income_order) * 2)
        visibility[income_idx * 2] = True
        visibility[income_idx * 2 + 1] = True
        
        buttons_list.append(dict(
            label=income_level,
            method='update',
            args=[{'visible': visibility},
                  {'title': f'Diabetes Rate by Healthcare Access Barriers - {income_level} Income'}]
        ))
    
    # Add all traces to figure
    for income_idx in range(len(income_order)):
        fig.add_trace(all_traces[income_idx * 2], row=1, col=1)
        fig.add_trace(all_traces[income_idx * 2 + 1], row=1, col=2)
    ...
    fig.update_layout(
        updatemenus=[dict(
            active=4,  # Default to $25k-$35k (middle income)
        ...
        title_text="Diabetes Rate by Healthcare Access Barriers - $25k-$35k Income",
        height=500,
        barmode='group',
        plot_bgcolor='white',
        paper_bgcolor='white',
...
def create_preexisting_factors_chart(df):
    """Create grouped bar chart for individual cardiovascular conditions."""
    df = df.copy()
    df.columns = df.columns.str.lower()
    ...
    for condition_name, condition_col in conditions.items():
        condition_data = df.groupby(condition_col)['diabetes_binary'].mean() * 100
        condition_counts = df.groupby(condition_col).size()
        
        no_rate = condition_data.get(0, 0)
        yes_rate = condition_data.get(1, 0)
        no_count = condition_counts.get(0, 0)
        yes_count = condition_counts.get(1, 0)
        ...
        fig.add_trace(go.Bar(
            ...
            textposition='outside',
            customdata=[[no_count]],
            hovertemplate=f'%{{x}}<br>No<br>Diabetes Rate: %{{y:.1f}}%<br>Count: %{{customdata[0][0]:,}}<extra></extra>',
            showlegend=(condition_name == 'Stroke'),
            legendgroup='No',
        ))
        ...
        fig.add_trace(go.Bar(
            ...
            text=[f"{yes_rate:.1f}%"],
            textposition='outside',
            customdata=[[yes_count]],
            hovertemplate=f'%{{x}}<br>Yes<br>Diabetes Rate: %{{y:.1f}}%<br>Count: %{{customdata[0][0]:,}}<extra></extra>',
            showlegend=(condition_name == 'Stroke'),
            legendgroup='Yes',
        ))
    
    fig.update_layout(
        title="Diabetes Rate by Individual Pre-Existing Conditions",
